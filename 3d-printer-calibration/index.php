...
<body class="app layout-default page-3d-printer-calibration">
    ...
    <!-- Import Bootstrap, Font Awesome and jQery from the CDN -->
...
`;
        ...
        function header() {
            ...
            var timestamp = today.toDateString(); // "Sun Jun 14 2020"
            // Home Template
            let header = `; Makers Mashup Startup GCode Generator (C) 2022 - Robert Mech
; Generated at ${timestamp} by https://www.layerfused.com/3d-printer-calibration
`;
            return header;
        }
        let footer = `; Footer - Reset extruder and go to absolute extrusion mode
G92 E0     ; Reset Extruder
G90        ; Reset Absolute Positioning
G0 Z4 F400    ; Move down just a bit
`;
        let homeTemplate = `G90           ; Set absolute positioning mode
...
G0 X${sgg_strip_x+40} Y${sgg_strip_y} Z5 F5000		; Move to position
G92 E0					; Reset Extruder
G1 E80 F300				; Extrude a kiss
G0 X${sgg_strip_x+5} Y${sgg_strip_y} Z0 F1000		; Wipe 
G0 X${sgg_strip_x+15} Y${sgg_strip_y} Z0 F1000		; Wipe 
G0 X${sgg_strip_x+5} Y${sgg_strip_y+5} Z-${sgg_strip_distance} F700	; Wipe 
G0 X${sgg_strip_x+15} Y${sgg_strip_y+5} Z-${sgg_strip_distance} F700	; Wipe 
G0 X${sgg_strip_x+5} Y${sgg_strip_y+10} Z0 F1000	; Wipe 
G0 X${sgg_strip_x+5} Y${sgg_strip_y+10} Z0 F1000	; Wipe 
G0 Z2 E-${sgg_strip_retraction} F300
M211 S1                 ; Re enable software endstops
G0 X${sgg_x_size/2} Y${sgg_x_size/2} Z4	F5000		; Return to center of bed
`;
            return kiss;
        }
